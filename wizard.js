/* --- ADX Wizard: Inheritance & Branch Activation --- */
const WIZARD = {
    step: 1,
    open: () => {
        const m = document.createElement('div');
        m.id = 'wizMod'; m.className = 'modal-overlay';
        m.innerHTML = `<div class="modal-content high-density"><div class="wizard-sidebar"><div class="wizard-progress-item active" id="s1">1. Practice</div><div class="wizard-progress-item" id="s2">2. Identity</div><div class="wizard-progress-item" id="s3">3. Access</div></div><div class="wizard-main"><div id="wizBody" style="padding:3rem; flex:1; overflow-y:auto;"></div><div class="wizard-footer"><button class="btn btn-outline" onclick="WIZARD.close()">Cancel</button><button class="btn btn-primary" onclick="WIZARD.next()">Next Step</button></div></div></div>`;
        document.body.appendChild(m); WIZARD.render();
    },
    render: () => {
        const b = document.getElementById('wizBody');
        if(WIZARD.step===1) b.innerHTML = `<h3>Location Inheritance</h3><div class="form-grid"><div class="form-group span-2"><label>Select Branch</label><select onchange="WIZARD.inherit(this.value)"><option value="">-- Select --</option><option value="new">+ Add New Branch</option><option value="PR1">Summit Musculoskeletal</option></select></div><div id="inhBox" class="inherited-data-box span-2">Choose "Add New Branch" to enter fresh regulatory data.</div></div>`;
        ...
        if(WIZARD.step===3) b.innerHTML = `<h3>Associated Access</h3><textarea style="width:100%; height:150px; padding:15px;" placeholder="nurse1@practice.com, admin1@practice.com"></textarea>`;
    },
    inherit: (val) => {
        const box = document.getElementById('inhBox');
        if(val === 'new') box.innerHTML = `<b>New Branch Activation</b><br><input type="text" placeholder="Branch Name" style="margin-top:10px; width:100%; padding:8px;">`;
