/* --- ADX Wizard: Gold Image --- */
const WIZARD = {
step: 1,
open: () => {
const m = document.createElement('div'); m.id = 'wizMod'; m.className = 'modal-overlay';
...
if(WIZARD.step===1) b.innerHTML = <h3>Location Inheritance</h3><div class="form-grid"><div class="form-group span-2"><label>Branch Selection</label><select onchange="WIZARD.inherit(this.value)"><option value="">-- Choose Option --</option><option value="new">+ Add New Branch</option><option value="PR1">Summit Musculoskeletal</option></select></div><div id="inhBox" class="inherited-data-box span-2">Select a branch or create a new location to pull regulatory data.</div></div>;
...
if(val === 'new') box.innerHTML = <b>New Branch Activation</b><br><input type="text" placeholder="Branch Name" style="width:100%; padding:10px; margin-top:10px;">;
else if(val) box.innerHTML = <b>Inherited:</b> XX-XXX4492 (Tax ID) | Level 1 MSK Tiering Applied;
},
